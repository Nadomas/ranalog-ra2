...
/// <summary>
/// S3-03 host spawn authority. Drains loopback spawn requests, validates blueprint schema,
/// assembles via shared <see cref="RobotSpawnService"/>, rebinds drive authority, and
/// publishes spawn/despawn events so the client receives the same logical graph.
/// </summary>
[DisallowMultipleComponent]
[DefaultExecutionOrder(-160)]
public sealed class RobotHostSpawner : MonoBehaviour
{
    [SerializeField] PhysicsTestLoopbackTransport transport;
    [SerializeField] PhysicsTestLocalAuthority authority;
    [SerializeField] PhysicsTestTransportHost transportHost;
    ...
    void RebindAuthority()
    {
        ...
        var tracked = new PhysicsTestTransportHost.TrackedRobot[live.Count];
        for (var i = 0; i < live.Count; i++)
        {
            var inst = live[i];
            ...
            tracked[i] = new PhysicsTestTransportHost.TrackedRobot
            {
                RobotId = inst.RobotId,
                Drive = inst.Drive
            };
        ...
        if (transportHost != null)
            transportHost.Configure(transport, authority, tracked);
    }
}
