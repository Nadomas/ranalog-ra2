        ...
        public static object GetWindows()
        {
            ...
            foreach (var window in allWindows)
            {
                ...
                try
                {
                    windowInfoList.Add(new
                    {
                        ...
                        instanceId = UnityMCP.Editor.EntityIdCompat.IdOf(window),
                        position = new
                        {
                            x = window.position.x,
                            y = window.position.y,
            ...
            return new
            {
                ...
                focusedWindow = focusedWindow != null ? new
                {
                    ...
                    instanceId = UnityMCP.Editor.EntityIdCompat.IdOf(focusedWindow)
                } : null,
                mouseOverWindow = mouseOverWindow != null ? new
                {
                    title = mouseOverWindow.titleContent?.text ?? "Unknown",
                    type = mouseOverWindow.GetType().Name,
                    instanceId = UnityMCP.Editor.EntityIdCompat.IdOf(mouseOverWindow)
                } : null,
